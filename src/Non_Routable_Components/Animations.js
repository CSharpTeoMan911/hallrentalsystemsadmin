let navbar_size_monitor = undefined;

let animations = {};
let animations_values = {};
let animations_offsets = {};

function Gradient_Flutuation_Animation(
  element_id,
  min_offset,
  max_offset,
  rgb1,
...
export function Set_Gradient_Flutuation_Animation(
  element_id,
  min_offset,
  max_offset,
  rgb1,
  ...
  if (element_id != undefined || element_id != null) {
    ...
    animations[element_id] = setInterval(async () => {
      await Gradient_Flutuation_Animation(
        element_id,
        min_offset,
        max_offset,
        rgb1,
...
function Monitor_Navbar_Size() {
    try{
        if (navbar_size_monitor !== undefined) {
            clearInterval(navbar_size_monitor);
          }
          let navbar = document.getElementById("app_navbar").offsetHeight;
          let page = document.getElementById("page").offsetHeight;
          let background_image = document.getElementById("background_image");
          
          if(page + navbar > window.visualViewport.height)
          {
            let calc = navbar + page;
            background_image.style.height = "calc(" + calc + "px + 10px + 1vw)";
          }
          else{
...
setInterval(Monitor_Navbar_Size, 10);
